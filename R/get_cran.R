...
get_meta_cran <- function(packages) {
  title <- version <- date <- authors <- github_url <- rep(NA_character_, length(packages))
  url <- cran_url <- paste0("https://CRAN.R-project.org/package=",packages)
  for (i in seq_along(packages)) {
    meta <- pkgsearch::cran_package(packages[i])
    date[i] <- meta$date
    title[i] <- meta$Title
  ...
  tibble::tibble(package = packages, date = date, title = title,
